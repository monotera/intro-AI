...
% Sebastián Ruiz
% David Castillo
% Uso: ejemplo: busqueda([d,d,d,d],[i,i,i,i]) En este caso, el estado inicial (primer argumento) es el robot y las cajas estan en la derecha 
% y el estado final (segundo argumento) el robot y las cajas pasen a estar en la izquierda
%[Robot, caja amarilla, caja verde, caja roja]
%h1 = d h2 = i
% A - es la habitacion donde esta el robot
% B, C y D- son las cajas. Se le da el valor de v si el robot tiene en la pinza la caja 1 o 2 o 3, respectivamente
...
busqueda(Inicio, Meta) :-
    ...
    heuristica(Inicio, Meta, H,_,_,_,_),% Se calcula la heuristica del estado actual frente al estado final
...
costoRobot([X,_,_,_],[Y,_,_,_], Puntaje) :- 
    (X\==Y) , Puntaje is 1.

costoRobot([X,_,_,_],[Y,_,_,_], Puntaje) :- 
    (X==Y) , Puntaje is 0.

costoC1([_,X,_,_],[_,Y,_,_], Puntaje) :- 
    (X\==Y) , Puntaje is 1.

costoC1([_,X,_,_],[_,Y,_,_], Puntaje) :- 
    (X==Y) , Puntaje is 0.

costoC2([_,_,X,_],[_,_,Y,_], Puntaje) :- 
    (X\==Y) , Puntaje is 1.

costoC2([_,_,X,_],[_,_,Y,_], Puntaje) :- 
    (X==Y) , Puntaje is 0.

costoC3([_,_,_,X],[_,_,_,Y], Puntaje) :- 
    (X\==Y) , Puntaje is 1.
...
mover(Estado_guardado, Abierto, Cerrado,Child, Meta) :-
    ...
    G_new is G + 1,
    heuristica(Siguiente, Meta, H,_,_,_,_),
    F is G_new + H,
...
imprimir_solucion(Siguiente_guardado, _):-
    guardar_estado(Estado, nil, _,H,_,Siguiente_guardado),
    write(Estado+H), nl.

imprimir_solucion(Siguiente_guardado, Cerrado) :-
    guardar_estado(Estado, Padre, _,H,_, Siguiente_guardado),
